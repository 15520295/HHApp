...
public class LoadingProcessBar extends ProgressBar {
    private static final int INDETERMINATE_MAX = 100;
    ...
    public LoadingProcessBar(Context context, AttributeSet attrs) {
        this(context, attrs, -1);
    }

    public LoadingProcessBar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);

        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.LoadingProcessBar, defStyleAttr, 0);
        int backgroundColour;
        int progressColour;
        try {
            backgroundColour = ta.getColor(R.styleable.LoadingProcessBar_backgroundColour, 0);
            progressColour = ta.getColor(R.styleable.LoadingProcessBar_progressColour, 0);
            int defaultDuration = context.getResources().getInteger(android.R.integer.config_mediumAnimTime);
            duration = ta.getInteger(R.styleable.LoadingProcessBar_duration, defaultDuration);
        } finally {
            ta.recycle();
        }
    ...
    public synchronized void setIndeterminate(boolean indeterminate) {
        ...
        stepCount = ValueAnimator.INFINITE;
    ...
    private Animator createIndeterminateAnimator() {
        ...
        Animator progressAnimator = getAnimator(SECONDARY_PROGRESS, new FastOutSlowInInterpolator());
        Animator secondaryProgressAnimator = getAnimator(PROGRESS, new FastOutLinearInInterpolator());
    ...
    private ObjectAnimator getAnimator(String propertyName, Interpolator interpolator) {
        ...
        progressAnimator.setRepeatCount(stepCount);
